...
Factura:</xsl:text>
    ...
    <xsl:text>
    empresa: </xsl:text><xsl:value-of select="empresa" />
    <xsl:text>
    calle: </xsl:text><xsl:value-of select="calle" />
    <xsl:text>
    localidad: </xsl:text><xsl:value-of select="localidad" />
    <xsl:text>
    CIF: </xsl:text><xsl:value-of select="CIF" />
    <xsl:text>
    telefono: </xsl:text><xsl:value-of select="telefono" />
    <xsl:text>
    fax: </xsl:text><xsl:value-of select="fax" />
    <xsl:text>
    Fecha: </xsl:text><xsl:value-of select="Fecha" />
    <xsl:text>
    pedido: </xsl:text><xsl:value-of select="pedido" />
    <xsl:text>
    forma_de_pago: </xsl:text><xsl:value-of select="forma_de_pago" />
  </xsl:template>
...
datos_cliente:</xsl:text>
    ...
    <xsl:text>
    nombre: </xsl:text><xsl:value-of select="nombre" />
    <xsl:text>
    dirección: </xsl:text><xsl:value-of select="dirección" />
    <xsl:text>
    población: </xsl:text><xsl:value-of select="población" />
    <xsl:text>
    codigo_postal: </xsl:text><xsl:value-of select="codigo_postal" />
    <xsl:text>
    provincia: </xsl:text><xsl:value-of select="provincia" />
  </xsl:template>
